async function load()
{
  ...
  const upcomingEventsContainer = document.getElementById("upcomingEvent");
  upcomingEventsContainer.classList.add("row","row-cols-sm-1","row-cols-md-2" , "row-cols-lg-3","row-cols-xl-4");
  ...
  let datosFiltrados = datos.filter(el => el.estimate)  
  cargarHtml(datosFiltrados, upcomingEventsContainer);
  ...
  function verifyCheck() {
    ...
    cargarHtml(cardsCheked, upcomingEventsContainer);
    filterAll(datosFiltrados, selectChecked, searchInput,upcomingEventsContainer)
  }
  ...
  //Search input
    ...
    cargarHtml(nuevo,upcomingEventsContainer);
    filterAll(datosFiltrados, selectChecked, searchInput,upcomingEventsContainer)
  })
